...
interface CartRule { id: string; name: string; description: string | null; discount_type: string; discount_amount: number; apply_to_shipping: boolean; stop_rules_processing: boolean; is_active: boolean; valid_from: string | null; valid_until: string | null; sort_order: number; }
interface TypeOption { value: string; label: string; }
interface Props { rule: CartRule; discountTypeOptions: TypeOption[]; }

export default function CartRuleEdit({ rule, discountTypeOptions }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        name: rule.name, description: rule.description ?? '',
        discount_type: rule.discount_type, discount_amount: String(rule.discount_amount),
        apply_to_shipping: rule.apply_to_shipping, stop_rules_processing: rule.stop_rules_processing,
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <h1 className="text-2xl font-semibold">Edit Cart Rule</h1>
            </div>
            <form onSubmit={e => { e.preventDefault(); put(`/admin/promotions/cart-rules/${rule.id}`); }} className="max-w-md space-y-4">
                ...
                <div className="space-y-1"><Label>Sort Order</Label><Input type="number" value={data.sort_order} onChange={e => setData('sort_order', e.target.value)} className="w-28" /></div>
                <div className="space-y-2">
                    <div className="flex items-center gap-3"><Switch checked={data.apply_to_shipping} onCheckedChange={v => setData('apply_to_shipping', v)} id="apply_shipping" /><Label htmlFor="apply_shipping">Apply to shipping</Label></div>
                    <div className="flex items-center gap-3"><Switch checked={data.stop_rules_processing} onCheckedChange={v => setData('stop_rules_processing', v)} id="stop_rules" /><Label htmlFor="stop_rules">Stop further rules</Label></div>

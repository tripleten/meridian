...
interface Props { typeOptions: TypeOption[]; }

export default function CouponCreate({ typeOptions }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        code: '', description: '', type: 'multi_use',
        usage_limit: '', usage_limit_per_customer: '',
        is_active: true as boolean, valid_from: '', valid_until: '',
    });
    ...
    return (
        <AdminLayout>
            ...
            <form onSubmit={e => { e.preventDefault(); post('/admin/promotions/coupons'); }} className="max-w-md space-y-4">
                <div className="space-y-1">
                    <Label>Code *</Label>
                    <Input value={data.code} onChange={e => setData('code', e.target.value.toUpperCase())} placeholder="SAVE20" required />
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        ...
                        <Input type="number" min="1" value={data.usage_limit} onChange={e => setData('usage_limit', e.target.value)} placeholder="Unlimited" />
                    </div>
                    <div className="space-y-1">
                        <Label>Per Customer</Label>
                        <Input type="number" min="1" value={data.usage_limit_per_customer} onChange={e => setData('usage_limit_per_customer', e.target.value)} placeholder="Unlimited" />
                    </div>
                </div>
                <div className="grid grid-cols-2 gap-4">

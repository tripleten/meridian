...
{
    public function handle(UpdateCartRuleCommand $command): EloquentCartRule
    {
        $rule = EloquentCartRule::find($command->id);

        if ($rule === null) {
            throw new DomainException("Cart rule '{$command->id}' not found.");
        }
